...
`ifdef PCIE_ASSERT
  ...
  always_ff @(posedge clk) begin
    if (rst_n) begin
      ...
      if (overflow_event_o)
        assert (!do_push) else $error("sync_fifo: push accepted while full");
      if (underflow_event_o)
        assert (!do_pop) else $error("sync_fifo: pop accepted while empty");
      if (do_pop)
        assert (count_q > '0 || do_push)
          else $error("sync_fifo: pop from empty fifo");
    end
  end
`endif
